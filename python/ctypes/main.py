# clang -I../../cpp/include -fPIC -shared misc.c -o misc.so  && python main.py

from ctypes import *
lib = CDLL("misc.so", RTLD_GLOBAL)
...
populate_array_with_junk = lib.populate_array_with_junk
populate_array_with_junk.argtypes = [POINTER(c_int), c_int]
populate_array_with_junk.restype = None
...
if __name__ == "__main__":
    ...
    # make a c array in python to pass to our glorious c code
    count = 1000
    values = [0] * count
    arr = c_array(c_int, values)
    # run said glorious c code
    populate_array_with_junk(arr, count)

    # print out the glorious results
    print("\n")
    print("omg this array was populated by c code")
    print("-----------------------------------------------")
