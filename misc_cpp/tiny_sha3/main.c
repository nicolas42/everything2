...
void my_test(void)
{
    char *msg = "SHA-3 (Secure Hash Algorithm 3) is the latest member of the Secure Hash Algorithm family of standards, released by NIST on August 5, 2015.[4][5][6] Although part of the same series of standards, SHA-3 is internally different from the MD5-like structure of SHA-1 and SHA-2."
"SHA-3 is a subset of the broader cryptographic primitive family Keccak (/ˈkɛtʃæk/ or /ˈkɛtʃɑːk/),[7][8] designed by Guido Bertoni, Joan Daemen, Michaël Peeters, and Gilles Van Assche, building upon RadioGatún. Keccak's authors have proposed additional uses for the function, not (yet) standardized by NIST, including a stream cipher, an authenticated encryption system, a \"tree\" hashing scheme for faster hashing on certain architectures,[9][10] and AEAD ciphers Keyak and Ketje.[11][12]"
...
"The creators of the Keccak algorithms and the SHA-3 functions suggest using the faster function KangarooTwelve with adjusted parameters and a new tree hashing mode without extra overhead for small message sizes.";
    ...
    char buf[512];
    int buf_len[] = { 32, 40, 64, 128, 256, 512, };
    for(int j=0;j<6;++j)
    {
        memset(buf, 0, sizeof buf);
        sha3(msg, msg_len, buf, buf_len[j]);
        for(int i=0;i<strlen(buf);++i) printf("%02X", (uint8_t)buf[i]);
        printf("\n\n");
    }
    printf("\n\n");
